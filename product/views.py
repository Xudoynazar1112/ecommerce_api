...
class ProductViewSet(viewsets.ModelViewSet):
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        return Response({'products': serializer.data, 'related_products': related_serializer.data})

    @action(detail=False, methods=['get'])
    def top_rated(self, request):
        # Assuming a related name of "reviews" from Review model to Product model
        top_products = Product.objects.annotate(avg_rating=models.Avg('review__rating')).order_by('-avg_rating')[:5]
