...
from product.models import FlashSale, ProductViewHistory, Product
...
def check_flash_sale(request, product_id):
    ...
    upcoming_flash_sale = FlashSale.objects.filter(product=product,
                                                   start_date__lte=datetime.now() + timedelta(hours=24)).first()
    ...
    if user_viewed and upcoming_flash_sale:
        ...
        return Response({'message': f'This product will be on {discount}% off flash sale!', 'start_time': start_time,
                         'end_time': end_time})
    else:
